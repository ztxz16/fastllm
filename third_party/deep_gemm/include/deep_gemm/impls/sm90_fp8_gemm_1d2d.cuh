...
sm90_fp8_gemm_1d2d_impl(float* sfb, int* grouped_layout,
    ...
    // FastLLM also supports FP16 stores; the FP8/FP32 mainloop is unchanged.
    DG_STATIC_ASSERT((cute::is_same_v<cd_dtype_t, cutlass::bfloat16_t> or cute::is_same_v<cd_dtype_t, cutlass::half_t>), "Invalid C/D data dtype");
        ...
        while (scheduler.get_next_block(m_block_idx, n_block_idx)) {
            ...
            for (uint32_t local_idx = 0; local_idx < BLOCK_M / WAVE_BLOCK_M; ++ local_idx) {
                ...
                for (auto i = 0; i < WGMMA::kNumAccum / 4; ++ i) {
                    ...
                    if constexpr (cute::is_same_v<cd_dtype_t, cutlass::half_t>) {
                        ptx::SM90_U32x2_STSM_N<half2>::copy(
                            __floats2half2_rn(shifted_accum[i * 4 + 0], shifted_accum[i * 4 + 1]),
                            __floats2half2_rn(shifted_accum[i * 4 + 2], shifted_accum[i * 4 + 3]), smem_ptr);
                    } else {
