...
Once the API Server is ready, click **Open WebUI** to use the original WebUI inside Launcher, including saved conversations, Markdown, attachments, reasoning, and agents. It connects to the active model with the configured API key, without another WebUI process or port. The model management sidebar collapses while WebUI is open; use **Model management** in the top bar to return to launch profiles, downloads, logs, and hardware. Conversations use WebUI’s existing local storage and survive page refreshes. Stopping or switching models cancels active WebUI tasks.
