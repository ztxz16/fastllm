...
bool FastllmCudaFP8LinearAddCanRun(const Data &input, const Data &weight, const Data &bias,
    ...
    if ((input.dataType != DataType::FLOAT16 && input.dataType != DataType::BFLOAT16) ||
        ...
        weight.dataType != DataType::FP8_E4M3 || weight.dims.size() != 2 || weight.dims[0] != 5120 ||
        (weight.dims[1] != 6144 && weight.dims[1] != 17408) || weight.IsRepacked || weight.blockK != 1 ||
        weight.blockM != weight.dims[1] || weight.scales.size() != 5120 ||
        input.dims.back() != weight.dims[1] || input.Count(0) != uint64_t(weight.dims[1]) ||
        output.dims.back() != 5120 || output.Count(0) != 5120)
        return false;
    int device = 0;
    if (cudaGetDevice(&device) != cudaSuccess || !DenseCuda(input, device, 16) ||
        !DenseCuda(weight, device, 16) || !DenseCuda(output, device, 2))
        return false;
    if (!bias.dims.empty() && (bias.dataType != DataType::FLOAT32 || bias.dims != std::vector<int>{5120} ||
                               !DenseCuda(bias, device, 4)))
        return false;
    if (Overlap(input, input.Count(0) * 2, output, 10240) ||
        Overlap(weight, weight.Count(0), output, 10240) ||
        (!bias.dims.empty() && Overlap(bias, 20480, output, 10240)))
        return false;
    ...
    if (it == supported.end()) {
        ...
        auto status = cudaFuncGetAttributes(&attributes, fp8add::Kernel<half, 6144>);
...
void FastllmCudaFP8LinearAdd(Data &input, Data &weight, const Data &bias, Data &output) {
    FastllmCudaFP8E4M3EnsureScalesAndBiasOnDevice(weight, bias, 5120);
