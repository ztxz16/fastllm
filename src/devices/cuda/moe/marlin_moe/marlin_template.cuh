...
#include "dequant.cuh"
#include "marlin_mma.cuh"
#include "scalar_type.cuh"
...
template <const fastllm_marlin_moe_types::ScalarTypeId a_type_id,
          ...
          const fastllm_marlin_moe_types::ScalarTypeId c_type_id,
          const fastllm_marlin_moe_types::ScalarTypeId s_type_id,
...
template <int count, fastllm_marlin_moe_types::ScalarTypeId type_id>
...
template <fastllm_marlin_moe_types::ScalarTypeId type_id>
...
template <const fastllm_marlin_moe_types::ScalarTypeId a_type_id,  // A ScalarType id
          const fastllm_marlin_moe_types::ScalarTypeId b_type_id,  // B ScalarType id
          const fastllm_marlin_moe_types::ScalarTypeId c_type_id,  // C ScalarType id
          const fastllm_marlin_moe_types::ScalarTypeId s_type_id,  // B_SCALE ScalarType id
...
__global__ void Marlin(
  ...
  if constexpr (a_type_id != fastllm_marlin_moe_types::kFloat16.id() && a_type_id != fastllm_marlin_moe_types::kS8.id())
    return;
  #endif
  ...
  static constexpr auto num_bits =
      fastllm_marlin_moe_types::ScalarType::from_id(b_type_id).size_bits();
  // Disable use_fp16_accum for NVFP4 and cases when group_size == -1 &&
  // num_bits == 4
  constexpr bool use_fp16_accum =
      a_type_id == fastllm_marlin_moe_types::kFloat16.id() &&
      (!(b_type_id == fastllm_marlin_moe_types::kFE2M1f.id() && s_type_id == fastllm_marlin_moe_types::kFE4M3fn.id()) &&
  ...
  static constexpr auto a_type = fastllm_marlin_moe_types::ScalarType::from_id(a_type_id);
  static constexpr auto b_type = fastllm_marlin_moe_types::ScalarType::from_id(b_type_id);
  static constexpr auto c_type = fastllm_marlin_moe_types::ScalarType::from_id(c_type_id);
  static constexpr auto s_type = fastllm_marlin_moe_types::ScalarType::from_id(s_type_id);
  if constexpr (b_type == fastllm_marlin_moe_types::kFE2M1f) {
    static_assert(s_type == fastllm_marlin_moe_types::kFE4M3fn && group_blocks == 1 ||
                  s_type == fastllm_marlin_moe_types::kFE8M0fnu && group_blocks == 2);
  } else if constexpr (b_type == fastllm_marlin_moe_types::kFE4M3fn && s_type == fastllm_marlin_moe_types::kFE8M0fnu) {
    static_assert(group_blocks == 2);
  } else if constexpr (std::is_same<scalar_t, nv_bfloat16>::value) {
    static_assert(s_type == fastllm_marlin_moe_types::kBFloat16);
  } else if constexpr (std::is_same<scalar_t, half>::value) {
    static_assert(s_type == fastllm_marlin_moe_types::kFloat16);
  }
  ...
  constexpr bool has_zp = b_type == fastllm_marlin_moe_types::kU4 || b_type == fastllm_marlin_moe_types::kU8;
  constexpr bool is_int_type = b_type == fastllm_marlin_moe_types::kU4 || b_type == fastllm_marlin_moe_types::kU8 ||
                               b_type == fastllm_marlin_moe_types::kS4 || b_type == fastllm_marlin_moe_types::kS8 ||
                               b_type == fastllm_marlin_moe_types::kU4B8 || b_type == fastllm_marlin_moe_types::kU8B128;
  constexpr bool is_8bit_scale = s_type.size_bits() == 8;
  // see comments of dequant.h for more details
  constexpr bool dequant_skip_flop =
      is_a_8bit || (b_type == fastllm_marlin_moe_types::kFE4M3fn && !(s_type == fastllm_marlin_moe_types::kFE8M0fnu)) ||
      b_type == fastllm_marlin_moe_types::kFE2M1f && s_type == fastllm_marlin_moe_types::kFE4M3fn ||
      has_zp && !is_zp_float && !std::is_same<scalar_t, nv_bfloat16>::value ||
      has_zp && !is_zp_float && !(b_type == fastllm_marlin_moe_types::kU8);
  ...
  auto matmul = [&](int k, int pipe) {
    ...
    if constexpr (s_type == fastllm_marlin_moe_types::kFE4M3fn || s_type == fastllm_marlin_moe_types::kFE8M0fnu) {
  ...
  #pragma unroll
    for (int j = 0; j < 4; j++) {
      ...
      if constexpr (b_type_id == fastllm_marlin_moe_types::kFE2M1f.id()) {
  ...
  #pragma unroll
      ...
      if constexpr (group_blocks != -1) {
        if (group_blocks == 2 || k == 1) {
          if constexpr (a_type == fastllm_marlin_moe_types::kS8) {
            int2 s_vals[2];
            s_vals[0] = {
  ...
  #pragma unroll
            ...
            float2 s_vals[2];
            if constexpr (s_type_id != fastllm_marlin_moe_types::kFE8M0fnu.id()) {
