...
namespace fastllm {
    ...
    std::vector <int> MinimaxM2Model::ForwardBatch(int batch, const fastllm::Data &inputIds, const fastllm::Data &attentionMask,
                            const fastllm::Data &positionIds, std::vector<std::pair<Data, Data>> &pastKeyValues,
                            const GenerationConfig &generationConfig, const LastTokensManager &lastTokens,
        ...
        ToDataType(hiddenStates, this->moeAtype);
        ...
        for (int i = 0; i < block_cnt; i++) {
            ...
            if (weight.weight.find(mergeQkvWeightName) != weight.weight.end()
                ...
                ToDataType(attenInput, this->moeAtype);
            ...
            // 2. moe mlp
            if (weight.weight.find("model.layers." + std::to_string(i) + ".block_sparse_moe.w1.weight") != weight.weight.end()) {
                if (CanRunLinearEx(LinearExType::ExSilu)) {
                    LinearEx(attenInput, weight["model.layers." + std::to_string(i) + ".block_sparse_moe.w1.weight"], Data(), w1, LinearExType::ExSilu);
                } else {
                    Linear(attenInput, weight["model.layers." + std::to_string(i) + ".block_sparse_moe.w1.weight"], Data(), w1);
                    Silu(w1, w1);
                }
                Linear(attenInput, weight["model.layers." + std::to_string(i) + ".block_sparse_moe.w3.weight"], Data(), w3);
                MulTo(w1, w3);
                Linear(w1, weight["model.layers." + std::to_string(i) + ".block_sparse_moe.w2.weight"], Data(), w2);
...
// exit(0);
                    ToDataType(attenInput, this->moeAtype);
                    ToDataType(moeFinal, this->moeAtype);
                } else {
    ...
    std::vector <int> MinimaxM2Model::ForwardBatch(int batch,
                                               const Data &inputIds,
        ...
        for (int i = 0; i < block_cnt; i++) {
            ...
            if (weight.weight.find(mergeQkvWeightName) != weight.weight.end()
                ...
                if (alibiData.dims.size() == 0 && all1 && batch > 1) {
                    ...
                    AttentionBatch(qs, keys, values, masks, contexts, qs[0]->dims[0] / values[0]->dims[0], 1.0 / sqrt(head_dim), 1);
                } else {
                    if (alibiData.dims.size() == 0) {
            ...
            if (weight.weight.find("model.layers." + std::to_string(i) + ".block_sparse_moe.w1.weight") != weight.weight.end()) {
                ...
                // 这里是moe mlp
                std::string gateWeightName = "model.layers." + std::to_string(i) + ".block_sparse_moe.gate.weight";
                std::string gateBiasName = "model.layers." + std::to_string(i) + ".block_sparse_moe.gate.e_score_correction_bias";
