...
namespace fastllm {
    ...
    std::vector <float> GetInterLeavePowerOf3(int n) {
    ...
    std::vector <float> GetInterleave2(int n) {
        int base = 1;
        while (base < n) {
            base <<= 1;
        }
        if (base == n) {
            ...
            std::vector <float> ret = GetInterLeavePowerOf3(base / 2);
            std::vector <float> part2 = GetInterLeavePowerOf3(base);
    ...
    int MiniCpmModel::Forward(const fastllm::Data &inputIds, const fastllm::Data &attentionMask,
        ...
        if (this->weight.dicts["use_alibi"] == "1") {
            std::vector<float> alibi = GetInterleave2(num_attention_heads);
...
//auto st = std::chrono::system_clock::now();
    ...
    void MiniCpmModel::ResponseBatch(const std::vector<std::string> &inputs, std::vector<std::string> &outputs,
    ...
    std::string MiniCpmModel::MakeInput(const std::string &history, int round, const std::string &input) {
        return (round == 0 ? pre_prompt : history) + user_role + input + bot_role;
    }

    std::string MiniCpmModel::MakeHistory(const std::string &history, int round, const std::string &input, const std::string &output) {
        return (round == 0 ? pre_prompt : history) + user_role + input + bot_role + output + history_sep;
    }

    void MiniCpmModel::WarmUp() {
        printf("Warmup...\n");
    ...
    int MiniCpmModel::LaunchResponseTokens(const std::vector<int> &inputTokens,
                                         const GenerationConfig &generationConfig) {
        mainLoopLocker.lock();
