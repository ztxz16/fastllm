//
// Created by huangyuyang on 2/9/25.
//
...
namespace fastllm {
    ...
    std::pair<std::vector<float>, std::vector<float>> Glm4MOEModel::UpdateRotaryPosEmb(float base, float factor, int seqLen) {
    ...
    int Glm4MOEModel::Forward(const fastllm::Data &inputIds, const fastllm::Data &attentionMask,
    ...
    std::vector <int> Glm4MOEModel::ForwardBatch(int batch, const fastllm::Data &inputIds, const fastllm::Data &attentionMask,
                            const fastllm::Data &positionIds, std::vector<std::pair<Data, Data>> &pastKeyValues,
                            const GenerationConfig &generationConfig, const LastTokensManager &lastTokens,
    ...
    std::vector <int> Glm4MOEModel::ForwardBatch(int batch,
                                               const Data &inputIds,
    ...
    bool Glm4MOEModel::NeedAttentionMask(int qlen, int klen) {
        if (((qlen == 1) || (qlen >= 1024))) {
            return false;
        }
        return true;
    ...
    void Glm4MOEModel::FillLLMInputsBatch(std::vector<std::vector<float>> &inputTokens,
    ...
    std::string Glm4MOEModel::MakeInput(const std::string &history, int round, const std::string &input) {
        return (round == 0 ? pre_prompt : history) + user_role + input + bot_role;
    }

    std::string Glm4MOEModel::MakeHistory(const std::string &history, int round, const std::string &input, const std::string &output) {
