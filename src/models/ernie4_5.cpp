...
namespace fastllm {
    ...
    std::pair<std::vector<float>, std::vector<float>> Ernie4_5Model::UpdateRotaryPosEmb(float base, float factor, int seqLen) {
    ...
    int Ernie4_5Model::Forward(const fastllm::Data &inputIds, const fastllm::Data &attentionMask,
    ...
    std::vector <int> Ernie4_5Model::ForwardBatch(int batch, const fastllm::Data &inputIds, const fastllm::Data &attentionMask,
                            const fastllm::Data &positionIds, std::vector<std::pair<Data, Data>> &pastKeyValues,
                            const GenerationConfig &generationConfig, const LastTokensManager &lastTokens,
        ...
        if (weights.size() == 0) {
            ...
            for (int i = 0; i < block_cnt; i++) {
                if (weight["model.layers." + std::to_string(i) + ".mlp.shared_experts.gateup_proj.weight"].dims.size() > 0) {
        ...
        for (int i = 0; i < block_cnt; i++) {
            ...
            if (weight.weight.find(mergeQkvWeightName) != weight.weight.end()
                ...
                fastllm::NearlyRotatePosition2D(q, positionIds, *sinDataPtr, *cosDataPtr, rotary_dim);
                fastllm::NearlyRotatePosition2D(k, positionIds, *sinDataPtr, *cosDataPtr, rotary_dim);
    ...
    std::vector <int> Ernie4_5Model::ForwardBatch(int batch,
                                               const Data &inputIds,
        ...
        for (int i = 0; i < block_cnt; i++) {
            ...
            PermuteSelf(q, {1, 0, 2, 3});        
            PermuteSelf(k, {1, 0, 2, 3});
            fastllm::NearlyRotatePosition2D(q, allPositionIds, *sinDataPtr, *cosDataPtr, rotary_dim);
            fastllm::NearlyRotatePosition2D(k, allPositionIds, *sinDataPtr, *cosDataPtr, rotary_dim);
            PermuteSelf(q, {1, 0, 2, 3});        
            PermuteSelf(k, {1, 0, 2, 3});
    ...
    bool Ernie4_5Model::NeedAttentionMask(int qlen, int klen) {
        if (((qlen == 1) || (qlen >= 1024))) {
            return false;
        }
        return true;
    ...
    void Ernie4_5Model::FillLLMInputsBatch(std::vector<std::vector<float>> &inputTokens,
    ...
    std::string Ernie4_5Model::MakeInput(const std::string &history, int round, const std::string &input) {
        return (round == 0 ? pre_prompt : history) + user_role + input + bot_role;
    }

    std::string Ernie4_5Model::MakeHistory(const std::string &history, int round, const std::string &input, const std::string &output) {
