...
namespace fastllm {
    ...
    MinimaxModel::MinimaxModel() {
        ...
        weight.linearNames = {
            ...
            "model.layers.*.self_attn.v_proj.weight", "model.layers.*.self_attn.mergeqkv.weight", "model.layers.*.self_attn.qkv_proj.weight",
            "model.layers.*.block_sparse_moe.*.weight",
            "model.layers.*.self_attn.output_gate.weight",
            "model.layers.*.self_attn.out_proj.weight"
        };
    }

    void MinimaxModel::InitParams() {
        basellm::InitParams();

        for (auto &c : this->weight.dicts["attn_type_list"]) {
            if (c == '0' || c == '1') {
                attn_type_list.push_back(c - '0');
            }
        }

        layernorm_full_attention_alpha = atof(this->weight.dicts["layernorm_full_attention_alpha"].c_str());
        layernorm_full_attention_beta = atof(this->weight.dicts["layernorm_full_attention_beta"].c_str());
        layernorm_linear_attention_alpha = atof(this->weight.dicts["layernorm_linear_attention_alpha"].c_str());
        layernorm_linear_attention_beta = atof(this->weight.dicts["layernorm_linear_attention_beta"].c_str());

        layernorm_mlp_alpha = atof(this->weight.dicts["layernorm_mlp_alpha"].c_str());
        layernorm_mlp_beta = atof(this->weight.dicts["layernorm_mlp_beta"].c_str());
        ...
        for (int i = 0; i < block_cnt; i++) {
            ...
            std::string mergeQkvWeightName = "model.layers." + std::to_string(i) + ".self_attn.qkv_proj.weight";
            std::string mergeQkvBiasName = "model.layers." + std::to_string(i) + ".self_attn.qkv_proj.bias";
        ...
        for (int i = 0; i < block_cnt; i++) {
            for (int j = 0; j < this->num_experts; j++) {
                ...
                std::string swigluWeightName = "model.layers." + std::to_string(i) + ".block_sparse_moe.experts." + std::to_string(j) + ".gateup_proj.weight";
    ...
    std::pair<std::vector<float>, std::vector<float>> MinimaxModel::UpdateRotaryPosEmb(float base, float factor, int seqLen) {
    ...
    int MinimaxModel::Forward(const fastllm::Data &inputIds, const fastllm::Data &attentionMask,
    ...
    std::vector <int> MinimaxModel::ForwardBatch(int batch, const fastllm::Data &inputIds, const fastllm::Data &attentionMask,
                            const fastllm::Data &positionIds, std::vector<std::pair<Data, Data>> &pastKeyValues,
                            const GenerationConfig &generationConfig, const LastTokensManager &lastTokens,
        ...
        Data hiddenStates;
        Data attenInput;
        Data q, k, v, qk, kv, qkv, outputGate, curInput, curOutput;
        ...
        if (weights.size() == 0) {
            ...
            for (int i = 0; i < block_cnt; i++) {
                ...
                biass[i].push_back(nullptr);
                for (int j = 0; j < this->num_experts; j++) {
                    weights[i].push_back(&weight["model.layers." + std::to_string(i) + ".block_sparse_moe.experts." + std::to_string(j) + ".gateup_proj.weight"]);
        ...
        for (int i = 0; i < block_cnt; i++) {
            ...
            if (attn_type_list[i] == 0) {
                ...
                int n = num_attention_heads, block = 256;
                int cur = seqlen;

                Data &pastKey = pastKeyValues[i].first, &pastValue = pastKeyValues[i].second;
                bool isFirst = true;
                if (pastKey.expansionDims.size() == 0) {
                    pastKey.Expansion({1, this->max_positions, 1});
                    pastKey.Resize({1, cur, 1});
                } else {
                    isFirst = false;
                    pastKey.Resize({1, pastKey.dims[1] + 1, 1});
                }

                if (!isFirst) {
                    if (ratioDatas.find(i) == ratioDatas.end()) {
                        std::vector <float> slope = GetInterleave(n);
                        for (int i = 0; i < slope.size(); i++) {
                            slope[i] *= (1.0f - (float)idx / (float)(block_cnt - 1) + 1e-5);
                        }

                        std::vector <float> ratio;
                        ratio.resize(n * head_dim * head_dim);
                        for (int i = 0; i < n; i++) {
                            std::fill(ratio.begin() + i * head_dim * head_dim, 
                                    ratio.begin() + (i + 1) * head_dim * head_dim, 
                                    exp(-slope[i]));
                        }

                        ratioDatas[i] = new Data();
                        ratioDatas[i]->ToDevice(DataDevice::CPU);
                        ratioDatas[i]->CopyFrom(Data(DataType::FLOAT32, { 1, n, head_dim, head_dim }, ratio));
                    }

                    PermuteSelf(k, {0, 1, 3, 2});
                    MatMul(k, v, kv);

                    MulTo(pastValue, *ratioDatas[i]);
                    AddTo(pastValue, kv);

                    MatMul(q, pastValue, qkv);
                } else {
                    int numBlock = (cur + block - 1) / block;
                    if (qDecays.find(i) == qDecays.end()) {
                        std::vector <float> slope = GetInterleave(n);
                        for (int i = 0; i < slope.size(); i++) {
                            slope[i] *= (1.0f - (float)idx / (float)(block_cnt - 1) + 1e-5);
                        }

                        std::vector <float> q_decay, k_decay;
                        q_decay.resize(n * block * head_dim);
                        k_decay.resize(n * block * head_dim);
                        for (int i = 0; i < n; i++) {
                            for (int j = 0; j < block; j++) {
                                for (int k = 0; k < head_dim; k++) {
                                    q_decay[i * block * head_dim + j * head_dim + k] = exp(-slope[i] * (j + 1));
                                    k_decay[i * block * head_dim + j * head_dim + k] = exp(-slope[i] * (block - 1 - j));
                                }
                            }
                        }

                        qDecays[idx] = new Data();
                        qDecays[idx]->ToDevice(DataDevice::CPU);
                        qDecays[idx]->CopyFrom(Data(DataType::FLOAT32, { 1, n, block, head_dim}, q_decay));

                        kDecays[idx] = new Data();
                        kDecays[idx]->ToDevice(DataDevice::CPU);
                        kDecays[idx]->CopyFrom(Data(DataType::FLOAT32, { 1, n, block, head_dim}, k_decay));

                        std::vector <float> diag_decay;
                        diag_decay.resize(n * block * block);
                        for (int i = 0; i < n; i++) {
                            for (int j = 0; j < block; j++) {
                                for (int k = 0; k < block; k++) {
                                    diag_decay[i * block * block + j * block + k] = exp(j < k ? -1e100 : slope[i] * (k - j));
                                }
                            }
                        }

                        diagDecays[idx] = new Data();
                        diagDecays[idx]->ToDevice(DataDevice::CPU);
                        diagDecays[idx]->CopyFrom(Data(DataType::FLOAT32, { 1, n, block, block }, diag_decay));

                        std::vector <float> block_decay;
                        block_decay.resize(slope.size() * head_dim * head_dim);
                        for (int i = 0; i < slope.size(); i++) {
                            std::fill(block_decay.begin() + i * head_dim * head_dim, 
                                    block_decay.begin() + (i + 1) * head_dim * head_dim, 
                                    exp(-slope[i] * block));
                        }
                        
                        blockDecays[idx] = new Data();
                        blockDecays[idx]->ToDevice(DataDevice::CPU);
                        blockDecays[idx]->CopyFrom(Data(DataType::FLOAT32, {1, n, head_dim, head_dim }, block_decay));
                    }
                    ...
                    for (int i = 0; i < numBlock; i++) {
                        int st = i * block;
                        int end = std::min((i + 1) * block, cur);
                        int m = end - st;

                        Data curQ, curK, curV, curQi;
                        Split(q, 2, st, end, curQ);
                        Split(k, 2, st, end, curK);
                        Split(v, 2, st, end, curV);
                        Data curKDecay, curQDecay;
                        Split(*kDecays[idx], 2, block - m, block, curKDecay);
                        Split(*qDecays[idx], 2, 0, m, curQDecay);

                        Data curBlockDecay, curDiagDecay0, curDiagDecay1;
                        Data *curBlockDecayPointer = blockDecays[idx];
                        Data *curDiagDecayPointer = diagDecays[idx];
                        if (m != block) {
                            std::vector <float> slope = GetInterleave(n);
                            for (int i = 0; i < slope.size(); i++) {
                                slope[i] *= (1.0f - (float)idx / (float)(block_cnt - 1) + 1e-5);
                            ...
                            curBlockDecay.ToDevice(DataDevice::CPU);
                            curBlockDecay.CopyFrom(Data(DataType::FLOAT32, {1, n, head_dim, head_dim }, block_decay));
                            curBlockDecayPointer = &curBlockDecay;

                            Split(*diagDecays[idx], 2, 0, m, curDiagDecay0);
                            Split(curDiagDecay0, 3, 0, m, curDiagDecay1);
                            curDiagDecayPointer = &curDiagDecay1;
                        }

                        if (i == 0) {
                            MatMulTransB(curQ, curK, qk);
                            MulTo(qk, *curDiagDecayPointer);
                            MatMul(qk, curV, qkv);
                            MulTo(curK, curKDecay);
                            PermuteSelf(curK, {0, 1, 3, 2});
                            MatMul(curK, curV, pastValue);
                        } else {                            
                            Mul(curQ, 1.0f, curQi);
                            MulTo(curQi, curQDecay);
                            MatMul(curQi, pastValue, qkvNoneDiag);

                            MatMulTransB(curQ, curK, qk);
                            MulTo(qk, *curDiagDecayPointer);

                            Data curQKV, temp;
                            MatMul(qk, curV, curQKV);
                            AddTo(curQKV, qkvNoneDiag);

                            Mul(qkv, 1.0f, temp);
                            Cat(temp, curQKV, 2, qkv);

                            Data curKV;
                            MulTo(curK, curKDecay);
                            PermuteSelf(curK, {0, 1, 3, 2});
                            MatMul(curK, curV, curKV);
                            MulTo(pastValue, *curBlockDecayPointer);

                            AddTo(pastValue, curKV);
                        }
                    }
                }

                Data &output = qkv;
                PermuteSelf(output, {0, 2, 1, 3});
                output.Reshape({output.dims[0], output.dims[1], -1});
                    
                RMSNorm(output, this->weight["model.layers." + std::to_string(i) + ".self_attn.norm.weight"], rms_norm_eps, output);

                Linear(attenInputTemp, this->weight["model.layers." + std::to_string(i) + ".self_attn.output_gate.weight"], Data(), outputGate);
                Sigmoid(outputGate, outputGate);

                MulTo(output, outputGate);
                Linear(output, this->weight["model.layers." + std::to_string(i) + ".self_attn.out_proj.weight"], Data(), attenInput);

                ToDataType(attenInput, DataType::FLOAT32);
                Mul(attenInput, layernorm_linear_attention_beta, attenInput);
                AddTo(attenInput, attenInputTemp, layernorm_linear_attention_alpha);
                Mul(attenInput, 1.0f, hiddenStates);
                ...
                Data q_, qpass, k_, kpass;
                Split(q, -1, 0, rotary_dim, q_);
                Split(q, -1, rotary_dim, head_dim, qpass);
                fastllm::LlamaRotatePosition2D(q_, positionIds, *sinDataPtr, *cosDataPtr, rotary_dim);
                Cat(q_, qpass, -1, q);

                Split(k, -1, 0, rotary_dim, k_);
                Split(k, -1, rotary_dim, head_dim, kpass);
                fastllm::LlamaRotatePosition2D(k_, positionIds, *sinDataPtr, *cosDataPtr, rotary_dim);
                Cat(k_, kpass, -1, k);
                ...
                // 1.2 Attention
                // Attention(q, pastKey, pastValue, attentionMask, qkv, q.dims[0] / pastKey.dims[0], 1.0 / sqrt(head_dim), 2);

                if (true) {
                    Attention(q, pastKey, pastValue, Data(), qkv, q.dims[0] / pastKey.dims[0], 1.0 / sqrt(head_dim), 1);
                } else {
                    MatMulTransB(q, pastKey, qk, 1.0 / sqrt(head_dim), q.dims[0] / pastKey.dims[0]);
                    Softmax(qk, qk, -1);
                    MatMul(qk, pastValue, qkv, 1.0, qk.dims[0] / pastValue.dims[0]);
                }
                ...
                Mul(attenInput, layernorm_full_attention_beta, attenInput);
                AddTo(attenInput, attenInputTemp, layernorm_full_attention_alpha);
                Mul(attenInput, 1.0f, hiddenStates);
            }
            ...
            // 2. moe mlp
            {
                ...
                if (weight.weight.find("model.layers." + std::to_string(i) + ".block_sparse_moe.experts.0.gateup_proj.weight") != weight.weight.end() 
                    ...
                    for (int b = 0; b < batch * len; b++) {
                        ...
                        for (int j = 0; j < this->num_experts_per_tok; j++) {
                            ...
                            if (weight.weight.find("model.layers." + std::to_string(i) + ".block_sparse_moe.experts." + std::to_string(idx) + ".gateup_proj.weight") != weight.weight.end()) {
                                if (CanRunLinearEx(LinearExType::ExSwiglu)) {
                                    LinearEx(*currentData, weight["model.layers." + std::to_string(i) + ".block_sparse_moe.experts." + std::to_string(idx) + ".gateup_proj.weight"], Data(), w1, LinearExType::ExSwiglu);
                                } else {
                                    Linear(*currentData, weight["model.layers." + std::to_string(i) + ".block_sparse_moe.experts." + std::to_string(idx) + ".gateup_proj.weight"], Data(), w3);
                                    Swiglu(w3, w1);
                                }
                            } else {
                ...
                Mul(hiddenStates, layernorm_mlp_alpha, hiddenStates);
                AddTo(hiddenStates, tempMoeFinal, layernorm_mlp_beta);
            }
        }
        ...
        if (generationConfig.top_k <= 1) {
            // 禁用simple greedy
            ((GenerationConfig*)&generationConfig)->top_k = 20;
    ...
    std::vector <int> MinimaxModel::ForwardBatch(int batch,
                                               const Data &inputIds,
    ...
    bool MinimaxModel::NeedAttentionMask(int qlen, int klen) {
        if (((qlen == 1) || (qlen >= 1024))) {
            return false;
        }
        return true;
    ...
    void MinimaxModel::FillLLMInputsBatch(std::vector<std::vector<float>> &inputTokens,
    ...
    std::string MinimaxModel::MakeInput(const std::string &history, int round, const std::string &input) {
        return (round == 0 ? pre_prompt : history) + user_role + input + bot_role;
    }

    std::string MinimaxModel::MakeHistory(const std::string &history, int round, const std::string &input, const std::string &output) {
