//
// Created by huangyuyang on 7/1/25.
//
...
namespace fastllm {
    ...
    void PanguMOEModel::InitParams() {
        basellm::InitParams();

        n_shared_experts = atoi(this->weight.dicts["shared_expert_intermediate_size"].c_str()) / atoi(this->weight.dicts["moe_intermediate_size"].c_str());
    ...
    std::pair<std::vector<float>, std::vector<float>> PanguMOEModel::UpdateRotaryPosEmb(float base, float factor, int seqLen) {
    ...
    int PanguMOEModel::Forward(const fastllm::Data &inputIds, const fastllm::Data &attentionMask,
    ...
    std::vector <int> PanguMOEModel::ForwardBatch(int batch, const fastllm::Data &inputIds, const fastllm::Data &attentionMask,
                            const fastllm::Data &positionIds, std::vector<std::pair<Data, Data>> &pastKeyValues,
                            const GenerationConfig &generationConfig, const LastTokensManager &lastTokens,
        ...
        if (weights.size() == 0) {
            ...
            for (int i = 0; i < block_cnt; i++) {
                if (weight["model.layers." + std::to_string(i) + ".mlp.shared_expert.gateup_proj.weight"].dims.size() > 0) {
                    weights[i].push_back(&weight["model.layers." + std::to_string(i) + ".mlp.shared_expert.gateup_proj.weight"]);
                    weights[i].push_back(&weight["model.layers." + std::to_string(i) + ".mlp.shared_expert.down_proj.weight"]);
                } else {
        ...
        for (int i = 0; i < block_cnt; i++) {
            ...
            if (weight.weight.find("model.layers." + std::to_string(i) + ".mlp.gate_proj.weight") != weight.weight.end()) {
                ...
                std::string routerScaleName = "model.layers." + std::to_string(i) + ".mlp.router_scale";
                this->weight[routerScaleName].ToDevice(DataDevice::CPU);
                float *routerScales = (float*)this->weight[routerScaleName].cpuData;
                ...
                if (weight.weight.find("model.layers." + std::to_string(i) + ".mlp.experts.0.gateup_proj.weight") != weight.weight.end() 
                    ...
                    for (int b = 0; b < batch * len; b++) {
                        ...
                        for (int i = 0; i < m; i += group) {
                            ...
                            v.push_back(std::make_pair(cur[idx], idx));
                        }
                        ...
                        for (int j = 0; j < this->num_experts_per_tok; j++) {
                            int idx = v[j].second;
                            float value = cur[idx] * routerScales[idx];
                        ...
                        if (weight.weight.find("model.layers." + std::to_string(i) + ".mlp.shared_expert.gateup_proj.weight") != weight.weight.end()) {
                            if (CanRunLinearEx(LinearExType::ExSwiglu)) {
    ...
    std::vector <int> PanguMOEModel::ForwardBatch(int batch,
                                               const Data &inputIds,
    ...
    bool PanguMOEModel::NeedAttentionMask(int qlen, int klen) {
        if (((qlen == 1) || (qlen >= 1024))) {
            return false;
        }
        return true;
    ...
    void PanguMOEModel::FillLLMInputsBatch(std::vector<std::vector<float>> &inputTokens,
    ...
    std::string PanguMOEModel::MakeInput(const std::string &history, int round, const std::string &input) {
        return (round == 0 ? pre_prompt : history) + user_role + input + bot_role;
    }

    std::string PanguMOEModel::MakeHistory(const std::string &history, int round, const std::string &input, const std::string &output) {
