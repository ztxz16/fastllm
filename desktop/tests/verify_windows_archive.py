...
def main():
    ...
    with zipfile.ZipFile(args.archive) as bundle:
        ...
        entries = [line.split("  ", 1)[::-1] for line in
                   bundle.read("FastLLM/MANIFEST.sha256").decode("utf-8").splitlines()]
        manifest = dict(entries)
        require(len(entries) == len(manifest), "Duplicate manifest entries")
        require(set(names) == {"FastLLM/" + name for name in manifest} | {"FastLLM/MANIFEST.sha256"}, "ZIP/manifest members differ")
        ...
        for name in ("FastLLM-Launcher.exe", "resources/app/main.js", "ftllm/runtime/python.exe"):
            require("FastLLM/" + name in names, f"Missing desktop payload: {name}")
        require("FastLLM/resources/default_app.asar" not in names, "Stock Electron demo in archive")
        require("FastLLM/ftllm/Launch.cmd" not in names, "Browser shortcut in desktop archive")
        require(not any(name.lower().endswith("/nvcuda.dll") for name in names), "NVIDIA driver must not be bundled")
        info = json.loads(bundle.read("FastLLM/BUILD-INFO.json"))
        for name, expected in info["desktop"]["application_sha256"].items():
            require(manifest["resources/app/" + name] == expected, f"Desktop source hash mismatch: {name}")
