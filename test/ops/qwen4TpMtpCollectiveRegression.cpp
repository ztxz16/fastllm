...
namespace {
#ifndef FASTLLM_TEST_TP_RANKS
#define FASTLLM_TEST_TP_RANKS 4
#endif
constexpr int kRanks = FASTLLM_TEST_TP_RANKS;
...
int main() {
    ...
    if (cudaGetDeviceCount(&count) != cudaSuccess || count < kRanks) {
        std::cout << "SKIP: requires at least " << kRanks << " CUDA GPUs\n";
        return 77;
    }
    std::vector<int> devices;
    for (int rank = 0; rank < kRanks; ++rank) devices.push_back(rank);
    Require(FastllmInitNccl(devices), "collective initialization");
    Barrier barrier;
    ...
    for (int rank = 0; rank < kRanks; ++rank) {
        workers.emplace_back([&, rank] {
            ...
            Data tokens(fastllm::INT32, {kSlots}), values(FLOAT32, {kSlots});
            ...
            for (int slot = 0; slot < kSlots; ++slot) {
                barrier.Wait();
                Require(FastllmCudaGraphBeginCapture(), "begin graph capture");
                run(slot);
                void *graph = nullptr;
                Require(FastllmCudaGraphEndCapture(&graph), "end graph capture");
                graphs[slot] = static_cast<cudaGraph_t>(graph);
                barrier.Wait();
    ...
    std::cout << "PASS: TP" << kRanks << " AllGather and device merge; 60 changing inputs, "
