    ...
    for (DataType type : {FLOAT32, FLOAT16, BFLOAT16}) {
        for (const Shape &shape : shapes) {
            for (int pattern = 0; pattern < 3; ++pattern) {
                ...
                Data selected(fastllm::INT32, {shape.rows, budget / ratio});
                Data reference(fastllm::INT32, {shape.rows, width});
                Data output(fastllm::INT32, {shape.rows, width});
                Data meta(fastllm::INT32, {1});
    ...
    for (DataType type : {FLOAT32, FLOAT16, BFLOAT16}) {
        for (int dim : {7, 128}) { // Scalar and vectorized FP16 gather.
            for (int sequence : {1, 2, 3, 4, 9}) {
                ...
                Data indices(fastllm::INT32, {sequence, capacity}), meta(fastllm::INT32, {1});
                indices.Allocate();
        ...
        for (int capacity : {512, 1281, 2048, 2049, 4096, 4097, 8192, 8193,
            ...
            for (int rows : {1, 4}) {
                for (int pattern = 0; pattern < 3; pattern++) {
                    ...
                    Data selected(fastllm::INT32, {rows, selectedK});
                    Data indices(fastllm::INT32, {rows, width});
                    Data meta(fastllm::INT32, {1});
