...
namespace {
bool benchmark = false, capture = false;
...
void Run(int kvHeads, int length, int rows, int layout, bool tail) {
    ...
    auto launch = [&]() {
        Require(FastllmCudaHalfPagedAttentionBatchFastllmFallback(q,k,v,qs,ps,pi,ls,out,group,1.f/16), "attention failed");
    };
    if (capture) {
    ...
    if (benchmark) {
        ...
        printf("BENCH kv_heads=%d len=%d T=%d layout=%d us=%.3f\n",kvHeads,length,rows,layout,ms*1000/32);
...
int main(int argc, char **argv) {
    ...
    try {
        ...
        capture = mode == "--graph";
        bool quick = mode == "--quick" || capture;
        // Local KV head counts for TP1/2/3/4; this does not simulate collectives.
        for (int kvHeads : benchmark ? std::vector<int>{4} : std::vector<int>{1, 2, 4})
            for (int length : benchmark ? std::vector<int>{1024,8192} : quick ? std::vector<int>{129} : std::vector<int>{9,127,129,1024,4096,4099})
                for (int rows : benchmark ? std::vector<int>{2,6} : quick ? std::vector<int>{2,6} : std::vector<int>{1,2,3,4,5,6,8,9})
                    for (int layout = 0; layout < (benchmark ? 1 : 5); ++layout)
