...
static void runCase(int queries,int cached,int runtime,int window,int group,int kvHeads,int device,bool bench=false) {
    ...
    // Capture fixed pointers, then poison the output before each replay.
    setenv("FASTLLM_DFLASH_ATTENTION","1",1);
    ...
    CUDA(cudaGraphInstantiateWithFlags(&executable,graph,0));
    for(int repeat=0;repeat<3;repeat++) {
        CUDA(cudaMemsetAsync(o.cudaData,255,o.GetBytes(),cudaStreamPerThread));
...
int main(int argc,char **argv) {
    try {
        ...
        if(!((prop.major==7 && prop.minor==5) || prop.major>=8)) return 77;
        ...
        for(int q:{1,2,3,4,5,6,7,8,16}) for(int cached:{0,1,127,128,129,2047,2048,2177})
        ...
        for(int q=1;q<=8;q++) for(int cached:{128,512,1024,2048,2559})
            runCase(q,cached,q,2048,4,8,device,!std::getenv("DFLASH_ATTN_NO_BENCH"));
        CUDA(cudaDeviceSynchronize());printf("PASS DFlash attention SM75/SM80+ device=%d\n",device);
