...
int main(){
 ...
 int badTotal=0;
 for(int mode:{1,2,0}){
  int M=1024,N=mode==1?34816:5120,K=mode==1?5120:17408,width=mode==1?N/2:N;
  ...
  setenv("FASTLLM_CUDA_NATIVE_NVFP4_FRESH","0",1);
  if(!Fp4(x,w,sf,global,nullptr,reference,M,N,N,K,mode))return 3;
  setenv("FASTLLM_CUDA_NATIVE_NVFP4_TMA","1",1);
  setenv("FASTLLM_CUDA_NATIVE_NVFP4_FRESH","1",1);
  ...
  std::vector<half>a(residual.size()),b(a.size());CK(cudaMemcpy(a.data(),reference,a.size()*2,cudaMemcpyDeviceToHost));CK(cudaMemcpy(b.data(),out,b.size()*2,cudaMemcpyDeviceToHost));int exact=0;int bad=0;float maxe=0;double se=0,sr=0;for(size_t i=0;i<a.size();i++){float v=__half2float(a[i]),u=__half2float(b[i]),e=fabs(v-u);exact+=memcmp(&a[i],&b[i],2)!=0;bad+=!std::isfinite(u)||e>.001f+.002f*fabs(v);maxe=fmax(maxe,e);se+=e*e;sr+=v*v;}printf("mode=%d bad=%d exact_mismatch=%d max_abs=%g rel_rms=%g\n",mode,bad,exact,maxe,sqrt(se/sr));badTotal+=bad;
  for(int small:{9,17,63,127,129,255,256,257,379,383,487,511,512,513,775,777,1023}) {
   ...
   int wrong=0,exactSmall=0;float maxSmall=0;
   for(size_t i=0;i<b.size();i++){
    ...
    exactSmall+=expected!=actual;wrong+=!std::isfinite(actual)||(i<size_t(small)*width?error>.001f+.002f*fabs(expected):error!=0);
    maxSmall=fmax(maxSmall,error);
   }
   printf("padded mode=%d M=%d bad=%d exact_mismatch=%d max_abs=%g\n",mode,small,wrong,exactSmall,maxSmall);badTotal+=wrong;
  }
