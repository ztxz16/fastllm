...
def test_swiglu(inputs=None):
    ...
    out = flm_ops.activation(input=to_tensor(inputs), activate_type="swiglu")
    mae = diff(np_out, out)
    assert mae <= 1e-6
    return out
...
def test_attention(q=None, k=None, v=None, mask=None, group=1, scale=1.0):
    if q is None:
        q = np.random.random(size=[1, 12, 4096])
        k = np.random.random(size=[1, 12, 4096])
        v = np.random.random(size=[1, 12, 4096])
...
def test_linear(inputs=None, 
    ...
    if not inputs:
        inputs = np.random.random(size=[1, 2, 4096])
    ...
    output = flm_ops.linear(to_tensor(inputs), weights, bias)
