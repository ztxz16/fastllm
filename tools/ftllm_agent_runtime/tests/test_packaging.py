...
@pytest.mark.skipif(
    not sys.platform.startswith("linux") or platform.machine().lower() != "x86_64",
    reason="the runtime wheel currently targets Linux x86-64",
)
def test_wheel_installs_binaries_in_platlib_without_cpython_abi(tmp_path):
    ...
    assert wheel.name.endswith("-py3-none-linux_x86_64.whl")
