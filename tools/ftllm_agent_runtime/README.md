...
The bridge intentionally disables Pi's built-in shell and mutation tools. It
exposes read-only project tools plus WebUI-mediated public-web tools:
...
Pi's built-in `bash`, `edit`, and `write` tools are disabled. Uploaded source
is copied into an isolated temporary directory before Pi starts. Web requests
are delegated to FastLLM's SSRF-protected `WebAgent` through an authenticated,
request-scoped localhost bridge; Pi never receives unrestricted shell access.
