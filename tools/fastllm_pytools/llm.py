...
if platform.system() == "Windows":
    fastllm_lib = ctypes.CDLL(
        os.path.join(os.path.split(os.path.realpath(__file__))[0], "fastllm_tools.dll"),
        winmode=0,
    )
elif platform.system() == "Darwin":
    fastllm_lib = ctypes.cdll.LoadLibrary(
        os.path.join(
            os.path.split(os.path.realpath(__file__))[0], "libfastllm_tools.dylib"
        )
    )
else:
    fastllm_lib = ctypes.cdll.LoadLibrary(
        os.path.join(
            os.path.split(os.path.realpath(__file__))[0], "libfastllm_tools.so"
        )
    )
...
fastllm_lib.token_decode.argtypes = [
    ctypes.c_int,
    ctypes.c_int,
    ctypes.c_int,
    ctypes.c_char_p,
...
fastllm_lib.token_encode_string.argtypes = [
    ctypes.c_int,
    ctypes.c_char_p,
    ctypes.c_int,
...
fastllm_lib.launch_response_llm_model.argtypes = [
    ctypes.c_int,
    ctypes.c_int,
    ctypes.c_void_p,
    ctypes.c_int,
...
fastllm_lib.fetch_response_logits_llm_model.argtypes = [
    ctypes.c_int,
    ctypes.c_int,
...
fastllm_lib.response_str_llm_model.argtypes = [
    ctypes.c_int,
    ctypes.c_char_p,
    ctypes.c_int,
    ctypes.c_bool,
...
fastllm_lib.launch_response_str_llm_model.argtype = [
    ctypes.c_int,
    ctypes.c_char_p,
    ctypes.c_int,
    ctypes.c_bool,
...
fastllm_lib.make_history_llm_model.argtype = [
    ctypes.c_int,
    ctypes.c_char_p,
    ctypes.c_int,
    ctypes.c_char_p,
...
fastllm_lib.make_input_llm_model.argtype = [
    ctypes.c_int,
    ctypes.c_char_p,
    ctypes.c_int,
    ctypes.c_char_p,
...
fastllm_lib.add_tokenizer_word_llm_model.argtype = [
    ctypes.c_int,
    ctypes.c_char_p,
    ctypes.c_float,
    ctypes.c_int,
...
fastllm_lib.set_device_map.argtype = [
    ctypes.c_int,
    ctypes.c_void_p,
    ctypes.c_char_p,
    ctypes.c_void_p,
...
def from_hf(model, tokenizer=None, dtype="float16"):
...
class model:
    def __init__(self, path: str, id: int = -99999):
        if id != -99999:
            self.model = id
        else:
    ...
    def get_prompt(self, query: str, history: List[Tuple[str, str]] = None) -> str:
        if not (history):
            history = []
        prompt = ""
        for i, (old_query, response) in enumerate(history):
            prompt = fastllm_lib.make_history_llm_model(
                self.model, prompt.encode(), i, old_query.encode(), response.encode()
            ).decode()
        prompt = fastllm_lib.make_input_llm_model(
            self.model, prompt.encode(), len(history), query.encode()
        ).decode()
        return prompt
    ...
    def tokenizer_encode_string(self, content: str) -> List[int]:
        ...
        if self.thread_local_obj.tokenizer_encode_string__output_buffer is None:
            self.thread_local_obj.tokenizer_encode_string__output_buffer = (
                ctypes.c_int * output_buffer_init_len
            )()

        buffer = self.thread_local_obj.tokenizer_encode_string__output_buffer
        buffer_len = len(buffer)
        result_len = fastllm_lib.token_encode_string(
            self.model, content.encode(), buffer_len, buffer
        )
        if result_len > buffer_len:
            if result_len > 10240:
                # 要处理的数据过长，使用一次性的buffer
                temp_buffer = (ctypes.c_int * result_len)()
                ret = fastllm_lib.token_encode_string(
                    self.model, content.encode(), result_len, temp_buffer
                )
                ...
                result_len = fastllm_lib.token_encode_string(
                    self.model, content.encode(), new_buffer_len, buffer
                )
    ...
    def tokenizer_decode_token(self, token_id: int) -> bytes:
        ...
        if self.thread_local_obj.tokenizer_decode_token__output_buffer is None:
            self.thread_local_obj.tokenizer_decode_token__output_buffer = (
                ctypes.create_string_buffer(output_buffer_init_len)
            )
    ...
    def stop_token_ctypes(self, stop_token_ids):
        if stop_token_ids is None:
            ...
            return ctypes.c_int(len(stop_token_ids)), (
                ctypes.c_int * len(stop_token_ids)
            )(*stop_token_ids)
    ...
    def response_logits(
        ...
        if tokenizer == None:
            handle = fastllm_lib.launch_response_str_llm_model(
                self.model,
                prompt.encode(),
                ctypes.c_int(1),
                ctypes.c_bool(False),
