...
def _runtime_info():
    ...
    if not shutil.which("bash", path=path):
        raise RuntimeError("Bash is required for Pi workspace commands.")
    return info
...
class AgentRuntimeInstaller:
    ...
    def _run(self, generation, install):
        try:
            ...
            self._update(generation, phase="unsupported", supported=False, error=str(error))
            return
        try:
            if install:
            ...
            if install and self._on_installed and not self._cancelled.is_set():
                self._on_installed()
            self._update(generation, phase="ready", available=True, piVersion=info["pi_version"], error="")
        except Exception as error:
            self._update(generation, phase="failed" if install else "missing", available=False, error=str(error))
