import {mountNativeAgent} from "./native-agent.js";

export function mountSessionAgent(options) {
  const {id, name, request, t, chooseDirectory} = options;
  const $ = suffix => document.getElementById(`${id}-${suffix}`);
  const rpc = (method, params = {}) => request(`/api/agents/${id}/rpc`, {method:"POST", body:JSON.stringify({method, params})});
  ...
  if ($("messages").hasAttribute("data-empty-message")) $("messages").dataset.emptyMessage = t("What would you like to work on?");
  const storageKey = `ftllm.${id}.sessions`;
  try {
  ...
  async function loadMarkdown() {
    ...
    try {
      ...
      url.searchParams.set(id, String(++markdownAttempt));
      ...
      if (!disposed) {
        markdownError = t("Could not load Markdown. Reopen the {name} page to retry.", {name});
  ...
  function controls() {
    ...
    $("turn-status").textContent = loading ? t("Loading conversation…") : busy ? t("{name} is working…", {name})
  ...
  function content(item) {
    ...
    if (item.type === "mcpToolCall" && item.tool) return [item.tool,
      JSON.stringify(item.arguments || {}, null, 2), item.result].filter(Boolean).join("\n\n");
    return JSON.stringify(item, null, 2);
  }
  function renderItem(item) {
    const scroller = $("messages"), bottom = scroller.scrollHeight - scroller.scrollTop - scroller.clientHeight < 80;
    const labels = {userMessage:"You", agentMessage:name, reasoning:"Thinking", commandExecution:"Command",
  ...
  function renderApprovals() {
    ...
    for (const pending of current) {
      ...
      const heading = document.createElement("strong"); heading.textContent = t("{name} needs your response", {name}); box.append(heading);
  ...
  async function poll() {
    ...
    try {
      const result = await request(`/api/agents/${id}/events?after=${cursor}&epoch=${encodeURIComponent(epoch)}`);
  ...
  return mountNativeAgent({...options, id, name,
